...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    # # people: will be populated by agent env cfg
    # person: ArticulationCfg = MISSING
    ...
    table = AssetBaseCfg(
        prim_path="{ENV_REGEX_NS}/Table",
        # init_state=AssetBaseCfg.InitialStateCfg(pos=[-0.1, 0, 0], rot=[0.707, 0, 0, 0.707]),
        init_state=AssetBaseCfg.InitialStateCfg(pos=[+0.05, 0, 0], rot=[0.707, 0, 0, 0.707]),
        spawn=UsdFileCfg(usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/SeattleLabTable/table_instanceable.usd",scale=[0.3,0.3,1.2]),
    )
    ...
    basket = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=[1.25, 0, 0.0], rot=[0.707, 0, 0, 0.707]),
    ...
    belt = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=[0.5, 0.45, -1.24], rot=[-0.707, 0, 0, 0.707]),
        spawn=UsdFileCfg(usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Conveyors/ConveyorBelt_A06.usd",scale=[0.9,0.9,0.7]),
    )
    ...
    person1 = AssetBaseCfg(
        ...
        # init_state=AssetBaseCfg.InitialStateCfg(pos=[-0.45, +0.15, 0.0], rot=[0.707, 0, 0, 0.707]),
        # init_state=AssetBaseCfg.InitialStateCfg(pos=[-0.45, +0.15, 0.0], rot=[0.707, 0, 0, 0.707]),
        init_state=AssetBaseCfg.InitialStateCfg(pos=[-0.8, +0.15, 0.0], rot=[0.707, 0, 0, 0.707]),
        # spawn=UsdFileCfg(usd_path=f"{ISAAC_NUCLEUS_DIR}/Robots/1X/Neo/Neo.usd",scale=[1.1,1.1,1.1],
        spawn=UsdFileCfg(usd_path="assets/person.usd",scale=[1.1,1.1,1.1],
    ...
    # collision_sensor_person0: ContactSensorCfg = ContactSensorCfg(
    #     prim_path="{ENV_REGEX_NS}/Object", update_period=0.0, history_length=6, debug_vis=True
    # )
    ############################
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        object_position = ObsTerm(func=mdp.object_position_in_robot_root_frame) # also containing some info about people
        obj_ee_tracking=ObsTerm(func=mdp.object_ee_distance_obs, params={"std": 0.1})
        Obj_bsk_tracking=ObsTerm(func=mdp.obj_bsk_distance_obs)
...
class EventCfg:
    ...
    reset_object_position = EventTerm(
        ...
        mode="reset",
        params={
            # "pose_range": {"x": (0.25, 0.5), "y": (-0.25, +0.25), "z": (0.0, 0.0)}, 
    ...
    ############
    # people_moving = EventTerm(
    #     func=mdp.moving_person,
    #     mode="interval",
    #     interval_range_s=(0.5, 0.5),
    #     params={
            
    #         "velocity_range": {"x": (0.0,0.0), "y": (+0.2,+0.2), "z": (+0.0, +0.0), "roll": (0.0,0.0), "pitch": (0.0,0.0), "yaw": (0.0,0.0)},
...
class RewardsCfg:
    ...
    # vel_check=RewTerm(func=mdp.obj_vel_release_check,params={"Vxy_velocity": 1},  weight=50)
    vel_check=RewTerm(func=mdp.obj_vel_release_check2,params={"Vxy_velocity": 0.85},  weight=50) ## user code
...
class LiftEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.sim.physx.gpu_total_aggregate_pairs_capacity = 32 * 1024 * 32
