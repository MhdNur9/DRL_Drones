...
def object_position_in_robot_root_frame(
    ...
    # person: RigidObject = env.scene["person1"]
   
    object_pos_w = object.data.root_pos_w[:, :3]
    # print("object_pos_w = ",object_pos_w)
    # person_pos_s = person.data
    # print("person_pos_s = ",person_pos_s)
...
def human_position_in_robot_root_frame(
    env: ManagerBasedRLEnv,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    object_cfg: SceneEntityCfg = SceneEntityCfg("object"),
    person_cfg: SceneEntityCfg = SceneEntityCfg("person"),
) -> torch.Tensor:
    """The position of the object in the robot's root frame."""
    robot: RigidObject = env.scene[robot_cfg.name]
    object: RigidObject = env.scene[object_cfg.name]
    #
    person: RigidObject = env.scene["person"]
    # print("person_pos_w = ",person.data.root_pos_w)

    person_pos_w = person.data.root_pos_w[:, :3]
    person_pos_b, _ = subtract_frame_transforms(
        robot.data.root_state_w[:, :3], robot.data.root_state_w[:, 3:7], person_pos_w
    )   
    #
    print("person_pos_b = ",person_pos_b)
