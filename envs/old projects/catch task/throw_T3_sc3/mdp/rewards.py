...
from envs.throw_T3_sc3.mdp.observations import bsk_pos_in_robot_root_frame
...
def object_is_lifted(
    ...
    env.extras['object_is_lifted']= torch.where(object.data.root_pos_w[:, 2] > minimal_height, 1.0, 0.0)
...
def reward_gripper_release_mid_throw(
    ...
    robot: Articulation=env.scene["person"]
    object: RigidObject = env.scene[object_cfg.name]
...
def obj_vel_release_check1(
    ...
    # print("obj_vel[:, :0] = ",obj_vel[:, :0])
    # print("obj_vel[:, :1] = ",obj_vel[:, :1])
    # to_basket_xy = bsk_pos[:, :2] - obj_pos[:, :2]
    # # Normalize both vectors
    # vel_unit = vel_xy / (torch.norm(vel_xy, dim=1, keepdim=True) + 1e-8)
    # basket_unit = to_basket_xy / (torch.norm(to_basket_xy, dim=1, keepdim=True) + 1e-8)
