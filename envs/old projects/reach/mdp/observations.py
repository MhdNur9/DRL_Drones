...
def ee_pos_error(env: ManagerBasedRLEnv, command_name: str,ee_frame_cfg: SceneEntityCfg = SceneEntityCfg("ee_frame"),robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    command = env.command_manager.get_command(command_name)
    # obtain the desired and current positions
    des_pos_b = command[:, :3]
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
